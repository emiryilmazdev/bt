...
(function ($) {
	...
	// Preloader js - React tarafından kontrol ediliyor
	// Sadece animasyonları başlat
	$(window).on("load", function () {
		// WOW ve GSAP animasyonlarını her durumda çalıştır
		wowController();
		gsapController();
	});
	...
	function gsapController() {
		...
		// Fade In on Scroll Animation - With perfect alignment
		function initFadeInRightOnScroll() {
			...
			const startOffset = 50;
			panels.forEach((panel, i) => {
				// Başlangıç pozisyonu: Sağdan geliyor
				gsap.set(panel, {
					xPercent: rtlValue(40),
					ease: "none",
				});
				
				// Bitiş pozisyonu: Natural CSS pozisyonu (mükemmel hizalama)
				gsap.to(panel, {
					xPercent: 0, // Natural CSS position - tüm kartlar aynı hizaya gelecek
					x: 0,        // Ekstra offset yok
					transform: "translateX(0%)", // Final transform temiz
					scrollTrigger: {
						trigger: panel,
